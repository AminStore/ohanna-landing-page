...
export default function TrackOrderPage() {
  ...
  const handleTrack = async (e: React.FormEvent) => {
    ...
    setOrder(null);
    try {
      const res = await fetch(`/api/track-order?id=${encodeURIComponent(orderId.trim())}&email=${encodeURIComponent(email.trim())}`);
        ...
        <div className="container mx-auto px-4 max-w-xl">
          ...
          {notFound && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              className="ohanna-card p-8 text-center">
              ...
              <p className="text-sm text-[#1B1B1B]/55 mb-4">We couldn't find an order matching that ID and email. Check your confirmation email for the correct details.</p>
          ...
          {order && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                      ...
                      <div key={step.key} className="flex flex-col items-center gap-1.5 flex-1">
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-colors ${
                          i <= currentStep
                            ...
                            : "bg-white border-[#1B1B1B]/15 text-[#1B1B1B]/25"
                        }`}>
                          <step.icon className="h-4 w-4" />
                        </div>
                        {i < STATUS_STEPS.length - 1 && (
                          <div className={`absolute h-0.5 w-full mt-4 ${i < currentStep ? "bg-[#C89D29]" : "bg-[#1B1B1B]/10"}`} />
                        )}
                      </div>
                    ))}
                  </div>
                  <div className="text-center mt-2">
                    <p className="font-black hieroglyph-font text-xs text-[#C89D29] tracking-wider">
              ...
              {/* Items */}
              <div className="ohanna-card p-6">
                <h3 className="font-black hieroglyph-font text-xs text-[#1B1B1B]/60 tracking-widest mb-4">ORDER ITEMS</h3>
                <div className="space-y-3">
                  {(order.items || []).map((item: any, i: number) => (
                    <div key={i} className="flex justify-between text-sm border-b border-[#1B1B1B]/6 pb-3 last:border-0 last:pb-0">
                      <span className="text-[#1B1B1B]/70">{item.product?.name} {item.size ? `(${item.size})` : ""} × {item.quantity}</span>
                      <span className="font-bold">EGP {((item.product?.price ?? 0) * item.quantity).toLocaleString("en-EG")}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <p className="text-center text-sm text-[#1B1B1B]/40 mt-8">
            Need help?{" "}
            <Link href="/contact" className="text-[#C89D29] font-semibold hover:underline">Contact support</Link>
          </p>
        </div>
      </main>

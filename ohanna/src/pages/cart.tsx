      ...
      <section className="py-12 bg-gradient-to-b from-[#E4D5B7]/40 to-[#FDF8EF] dark:from-[#2A1E14]/40 dark:to-[#1A1410]">
        <div className="container mx-auto px-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black hieroglyph-font text-[#1B1B1B] dark:text-[#FDF8EF]">YOUR CART</h1>
            <span className="text-sm text-[#1B1B1B]/40 dark:text-[#FDF8EF]/40 font-semibold">({items.length} item{items.length !== 1 ? "s" : ""})</span>
          </div>
        </div>
      </section>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            {items.length === 0 ? (
              <div className="ohanna-card text-center py-24 px-8">
                ...
                <p className="font-black hieroglyph-font text-[#1B1B1B]/30 dark:text-[#FDF8EF]/30 tracking-widest text-sm mb-2">YOUR CART IS EMPTY</p>
              ...
              <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-3">
                  {remaining > 0 && (
                    <div className="bg-[#C89D29]/10 dark:bg-[#C89D29]/15 border border-[#C89D29]/30 rounded-xl p-3.5 flex items-center gap-3 text-sm">
                      ...
                      <span className="text-[#1B1B1B]/70 dark:text-[#FDF8EF]/70">Add <strong className="text-[#1B1B1B] dark:text-[#FDF8EF]">{fmt(remaining)}</strong> more for free shipping!</span>
                    </div>
                  )}
                    ...
                    {items.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          {item.size && <p className="text-xs text-[#1B1B1B]/40 dark:text-[#FDF8EF]/40 mt-0.5">Size: {item.size}</p>}
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            <div className="flex items-center border border-[#1B1B1B]/15 dark:border-[#FDF8EF]/15 rounded-lg overflow-hidden">
                              <button onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.size)} className="w-8 h-8 flex items-center justify-center text-[#1B1B1B] dark:text-[#FDF8EF] hover:bg-[#1B1B1B]/5 dark:hover:bg-[#FDF8EF]/5 transition-colors"><Minus className="h-3 w-3" /></button>
                              ...
                              <button onClick={() => updateQuantity(item.product.id, item.quantity + 1, item.size)} className="w-8 h-8 flex items-center justify-center text-[#1B1B1B] dark:text-[#FDF8EF] hover:bg-[#1B1B1B]/5 dark:hover:bg-[#FDF8EF]/5 transition-colors"><Plus className="h-3 w-3" /></button>
                            </div>
                ...
                <div className="ohanna-card p-6 h-fit space-y-4">
                  <h2 className="font-black hieroglyph-font tracking-wider text-sm text-[#1B1B1B] dark:text-[#FDF8EF] pb-3 border-b border-[#1B1B1B]/8 dark:border-[#FDF8EF]/8">ORDER SUMMARY</h2>
                  <div className="border-t border-[#1B1B1B]/8 dark:border-[#FDF8EF]/8 pt-3 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-[#1B1B1B]/55 dark:text-[#FDF8EF]/55">Subtotal</span>
                      <span className="font-bold text-[#1B1B1B] dark:text-[#FDF8EF]">{fmt(total)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-[#1B1B1B]/55 dark:text-[#FDF8EF]/55">Shipping</span>
                      <span className={`font-bold ${shippingFee === 0 ? "text-[#1D4D4F] dark:text-[#4A9EA1]" : "text-[#1B1B1B] dark:text-[#FDF8EF]"}`}>{shippingFee === 0 ? "FREE" : fmt(shippingFee)}</span>
                    </div>
